...
protocol AudioRecordingManagerProtocol {
    func sendEmergencyMessage(user: String, coordinate: CLLocationCoordinate2D?, contactList: [EmergencyContact])
}

// Mock AudioRecordingManager for testing
class MockAudioRecordingManager: AudioRecordingManagerProtocol {
    var sendEmergencyMessageHandler: ((String, CLLocationCoordinate2D?, [EmergencyContact]) -> Void)?
    
    func sendEmergencyMessage(user: String, coordinate: CLLocationCoordinate2D?, contactList: [EmergencyContact]) {
        sendEmergencyMessageHandler?(user, coordinate, contactList)
    }
}
